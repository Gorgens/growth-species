...
Incremento de espécies de interesse que são comerciais
```{r incComercial, echo=FALSE, warning= FALSE, message=FALSE}
incrementoSpecies = incComercial%>%
  group_by(scientific.name) %>%
  filter(scientific.name %in%speciesOfInterest) %>%
...
incremento de espécies de teste 
...
knitr::kable(
  inner_join(arvHaEspecie, incrementoSpecies, by="scientific.name") %>%
    select(scientific.name, inc, incIQR,  rIncA, rIncIQR),
  caption = 'Resumo das Espécies de Interesse.'
    )
...
ggplot(incMedSpecies, aes(cc, incMediaAnual)) + 
  ...
  facet_wrap(~factor(scientific.name,levels=c("Dinizia excelsa","Eschweilera coriacea","Inga alba",
"Inga marginata","Jacaranda copaia","Laetia procera","Lecythis idatimon","Lecythis pisonis","Manilkara huberi",
"Nectandra cuspidata","Neea oppositifolia","Pourouma minor","Rinorea guianensis","Tetragastris altissima","Tetragastris panamensis")), ncol=4)

```


```{r ddIncComercial, echo=FALSE, message=FALSE, warning=FALSE}
incMedSpecies = incComercial %>%
  filter(scientific.name %in%speciesOfInterest) %>%
  group_by(scientific.name, cc) %>%
  summarise(incMediaAnual = mean(incAnual))
...
ggplot(incMedSpecies, aes(cc, incMediaAnual)) + 
  ...
  ylim(0, 2.5) +
    theme_bw() + theme(panel.grid.major = element_blank(),
                     ...
                     panel.background = element_blank(),
                     axis.line = element_line(colour = "black"))+
  facet_wrap(~factor(scientific.name,levels=c("Dinizia excelsa","Eschweilera coriacea",
"Jacaranda copaia","Laetia procera","Lecythis pisonis")), ncol=2)
...
ggplot(numeroArvoresAcumulado, aes(centroClasse, arvAcumuladas)) + geom_col() +
  xlab('Diameter distribution') + ylab('Accumulated trees per hectare') +
    theme_bw() + theme(panel.grid.major = element_blank(),
                     ...
                     panel.background = element_blank(),
                     axis.line = element_line(colour = "black"))+
  facet_wrap(~factor(speciesInteresse,levels=c("Dinizia excelsa","Eschweilera coriacea","Inga alba",
"Inga marginata","Jacaranda copaia","Laetia procera","Lecythis idatimon","Lecythis pisonis","Manilkara huberi",
"Nectandra cuspidata","Neea oppositifolia","Pourouma minor","Rinorea guianensis","Tetragastris altissima","Tetragastris panamensis")), ncol=4)
```


```{r graficoDDGrupoEcoAcumuladoComerciais, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = 'Distribuição diamétrica acumulada para as espécies comerciais.'}
numeroArvores = invMerged %>%
  filter(scientific.name %in%speciesOfInterest) %>%
  filter(comercial == 1) %>%
  group_by(area, year, plot, subplot, GrupoEco, scientific.name, cc) %>% 
...
ggplot(numeroArvoresAcumulado, aes(centroClasse, arvAcumuladas)) + geom_col() +
  xlab('Diameter distribution') + ylab('Accumulated trees per hectare') +
    theme_bw() + theme(panel.grid.major = element_blank(),
                     ...
                     panel.background = element_blank(),
                     axis.line = element_line(colour = "black"))+
  facet_wrap(~factor(speciesInteresse,levels=c("Dinizia excelsa","Eschweilera coriacea",
"Jacaranda copaia","Laetia procera","Lecythis pisonis")), ncol=2)
```
